...
#define BOOTPATTERN_DSPCODE_BIT 4
#define BOOTPATTERN_DSPPAR_BIT 8
#define BOOTPATTERN_PROGFIPPI_BIT 16
#define BOOTPATTERN_SETDACS_BIT 32
...
static void PixieBootModule(unsigned short module_, const char* ComFPGAConfigFile,
                            const char* SPFPGAConfigFile, const char* DSPCodeFile,
                            const char* DSPParFile, const char* DSPVarFile,
                            unsigned short BootPattern) {

    xia::pixie::module::module &module = *(crate.modules[module_]);
    ...
    crateGuard = new xia::pixie::crate::crate::guard(crate);
    xia::pixie::firmware::add(crate.firmware, comm_fw);
    xia::pixie::firmware::add(crate.firmware, fippi_fw);
    xia::pixie::firmware::add(crate.firmware, dsp_fw);
    xia::pixie::firmware::add(crate.firmware, dsp_var);
    ...
    module.add(crate.firmware[tag]);


    for (auto &fw : crate.firmware[tag]) {
        fw->load();
    }
    delete crateGuard;
...
PIXIE_EXPORT int PIXIE_API Pixie16BootModule(const char* ComFPGAConfigFile,
    ...
    try {
        if (ModNum == crate.num_modules) {
            ...
            for (unsigned short m = 0; m != crate.num_modules; ++m) {
                // PixieBootModule(*module, ComFPGAConfigFile, SPFPGAConfigFile, DSPCodeFile,
                //                 DSPParFile, DSPVarFile, BootPattern);
                bootThreads.push_back(std::thread(PixieBootModule, m, ComFPGAConfigFile, SPFPGAConfigFile, DSPCodeFile, DSPParFile, DSPVarFile, BootPattern));
            }
            for (auto &t : bootThreads) {
                t.join();
            }
        } else {
            PixieBootModule(ModNum, ComFPGAConfigFile, SPFPGAConfigFile,
